...
static void bond_ipsec_lag_call_inst(struct xfrm_state *xs,
				     ...
				     bool free_state)
{
	unsigned long bond_handle = READ_ONCE(xs->xso.offload_handle);
	unsigned long lower_handle = READ_ONCE(inst->lower_handle);
	struct net_device *bond_real_dev = READ_ONCE(xs->xso.real_dev);
	...
	WRITE_ONCE(xs->xso.real_dev, inst->real_dev);
	WRITE_ONCE(xs->xso.offload_handle, lower_handle);
	if (delete_state) {
	...
	if (free_state)
		WRITE_ONCE(inst->lower_handle, 0);

	WRITE_ONCE(xs->xso.real_dev, bond_real_dev);
	WRITE_ONCE(xs->xso.offload_handle, bond_handle);
}
				   ...
				   struct netlink_ext_ack *extack)
{
	...
	lower_handle = xs->xso.offload_handle;
	if (!lower_handle) {
		err = -EINVAL;
	...
	WRITE_ONCE(inst->lower_handle, lower_handle);
	inst->deleted = false;
	WRITE_ONCE(inst->added, true);
...
static void bond_ipsec_lag_flush_pending(struct bonding *bond)
{
	...
	list_for_each_entry_safe(ipsec, tmp, &bond->ipsec_list, list) {
		...
		/* The state is between successful offload add and XFRM hash
		 * insertion. Marking it DEAD under xfrm_state_lock makes
		 * xfrm_state_add()/xfrm_state_insert() fail instead of
		 * publishing a state whose lower hardware handles are being
		 * synchronously torn down here.
		 */
		xso = &xs->xso;
		list_del(&ipsec->list);
		bond_ipsec_lag_call_state(xs, ipsec, true, true);
					     ...
					     struct net_device *lower_dev)
{
	...
	list_for_each_entry_rcu(inst, &ipsec->inst_list, list) {
		if (READ_ONCE(inst->added) && inst->real_dev == lower_dev) {
			handle = READ_ONCE(inst->lower_handle);
			break;
		}
	}
...
static int bond_set_features(struct net_device *dev, netdev_features_t features)
{
	...
	flush = (dev->features & NETIF_F_HW_ESP) &&
