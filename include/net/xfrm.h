 ...
 * installed directly on lower_dev use xso.offload_handle. During upper-device
 * callbacks, xso.real_dev may identify the lower device whose callback is
 * running; use xso.offload_handle directly in that case. Other states owned by
 * an upper device are resolved through the owner's xdo_dev_state_lower_handle().
